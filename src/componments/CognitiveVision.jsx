...
import { cognitiveApi, limitWidthHeight } from '../cognitive/Cognitive.util'
...
const headers = new Headers({
  'Ocp-Apim-Subscription-Key': configVision.apiKey,
...
class CognitiveVision extends React.Component {
  ...
  handleCognitive = () => {
    const url = configVision.url + 'visualFeatures=Description&language=en'
    if (this.state.imageFile2.length > 0) {
      cognitiveApi(url, headers, this.state.imageFile2[0], errMessage).then(
        data => {
          if (data.err) {
            this.setState({
              message: errMessage
